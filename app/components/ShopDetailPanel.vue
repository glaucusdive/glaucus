<template>
  ...
  <div v-else-if="shopData" class="flex flex-col justify-between h-full gap-0 divide-y divide-zinc-300 dark:divide-zinc-700">
    <!-- Header -->
    <div class="flex flex-col justify-center z-40 w-full divide-y divide-zinc-300 dark:divide-zinc-700 shrink-0">
      <!-- Title -->
      <header class="flex flex-row justify-start items-stretch gap-0 divide-x divide-zinc-300 dark:divide-zinc-700">
        <div class="p-1 flex items-center">
          <button @click="$emit('close')"
            ...
            <X class="w-4 h-4 text-zinc-900 dark:text-white" />
          </button>
        </div>
        <!-- Image -->
        <div class="p-1 flex items-center">
          <div
            class="block bg-zinc-200 dark:bg-zinc-700 overflow-hidden rounded-sm min-w-8 w-8 h-auto aspect-square">
          </div>
        </div>
        <div class="p-1 grow flex items-center overflow-auto">
          <h1 class="text-sm font-medium p-0 leading-none w-full truncate text-zinc-900 dark:text-white">{{
      ...
      <div class="flex flex-row gap-1 items-center p-1 overflow-x-auto font-medium">
        <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="[
          'flex flex-row gap-2 rounded-sm p-2 px-3 w-fit text-xs cursor-pointer transition-color whitespace-nowrap',
          activeTab === tab.id
    ...
    <div class="w-full h-0 flex-1 overflow-y-auto">
      ...
      <div
        class="flex flex-col justify-start items-start gap-0 divide-y divide-zinc-300 dark:divide-zinc-700 w-full h-full overflow-y-auto">
        <!-- Tab Content -->
        ...
        <!-- Sidebar -->
        <div
          class="w-full p-2 h-auto order-2 sticky bottom-0 bg-zinc-50 dark:bg-zinc-900">
          <div class="h-full">
            <div class="flex flex-col gap-2">
              <!-- Book Now Button -->
              <div class="flex flex-col gap-2 p-6 bg-zinc-100 dark:bg-zinc-800 rounded-md order-2">
