        ...
        <div class="border border-neutral-200 rounded-xl h-full">
          <div class="flex flex-col justify-between h-full gap-0 divide-y divide-neutral-200">
            ...
            <div class="flex flex-col justify-center z-50 w-full divide-y divide-neutral-200">
              ...
              <!-- Tabs -->
              <div class="flex flex-row gap-1 items-center p-3 overflow-x-auto font-medium">
                      ...
                      <ul class="space-y-2">
                        <li v-if="shopData?.street_address || shopData?.locale || shopData?.country">
                          <a href="#" class="flex flex-row gap-4 items-center">
                            ...
                            <span>{{ [shopData?.street_address, shopData?.locale, shopData?.country].filter(Boolean).join(', ') }}</span>
                          </a>
                        </li>
                        <li v-if="shopData?.phone">
                          <a :href="`tel:${shopData.phone}`" class="flex flex-row gap-4 items-center">
                            <Phone class="min-w-4 max-w-4 h-4" />
                            <span>{{ shopData.phone }}</span>
                          </a>
                        </li>
                        <li v-if="shopData?.email">
                          <a :href="`mailto:${shopData.email}`" class="flex flex-row gap-4 items-center">
                            <Mail class="min-w-4 max-w-4 h-4" />
                            <span>{{ shopData.email }}</span>
                          </a>
                        </li>
                        <li v-if="shopData?.website_url">
                          <a :href="shopData.website_url" target="_blank" class="flex flex-row gap-4 items-center">
                            <Globe class="min-w-4 max-w-4 h-4" />
                            <span class="truncate">{{ shopData.website_url }}</span>
                          </a>
                        </li>
                      </ul>
                    </div>
...
// Fetch dive shop data for the first shop (or a specific one)
const { client } = useSupabase()

const { data: shopData, pending, error } = await useAsyncData('featured-diveshop', async () => {
  try {
    console.log('Fetching featured dive shop data...')
    ...
    console.log('Fetched featured shop data:', data)
    return data
  } catch (err) {
    console.error('Error fetching featured dive shop:', err)
    return null
  }
}, {
  server: false,
