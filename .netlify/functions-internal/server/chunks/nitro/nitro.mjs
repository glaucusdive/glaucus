...
const _inlineRuntimeConfig = {
  ...
  "public": {
    "supabaseUrl": "https://hyldglninkgngaweejmw.supabase.co",
    "supabaseKey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh5bGRnbG5pbmtnbmdhd2Vlam13Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ1OTI0OTksImV4cCI6MjA3MDE2ODQ5OX0.a-nD31mpBtGdRZ5g_5mLfd7xbA-TASqlrS6ouQ_4bWk"
  },
...
const _5CMEa8cICsr3GOGLlK2yZNcnAMPxpL4UamHehXQ29t4 = (function(nitro) {
...
const plugins = [
  _5CMEa8cICsr3GOGLlK2yZNcnAMPxpL4UamHehXQ29t4
];
...
function getNextBookingStep(payload) {
  ...
  for (let idx = 0; idx < numDivers; idx++) {
    ...
    if (d.numberOfDives === void 0 || d.numberOfDives === null || String(d.numberOfDives).trim() === "") return { step: "numberOfDives", diverIndex: idx, diverName: d.name };
...
async function buildDiveShopQuery(supabaseUrl, supabaseKey, filters) {
  ...
  let query = client.from("diveshops").select("*, country:countries(name), region:regions(name)");
  if ((_a = filters.country) == null ? void 0 : _a.trim()) {
    const { data: countries } = await client.from("countries").select("id").ilike("name", `%${filters.country.trim()}%`);
  ...
  if ((_b = filters.region) == null ? void 0 : _b.trim()) {
    const { data: regions } = await client.from("regions").select("id").ilike("name", `%${filters.region.trim()}%`);
...
async function getDiveSitesForShop(supabaseUrl, supabaseKey, shopId) {
  ...
  if (error || !data) return [];
  return data.filter((row) => row.dive_sites != null).map((row) => ({ id: row.dive_sites.id, name: row.dive_sites.name }));
}

async function getRentalEquipmentForShop(supabaseUrl, supabaseKey, shopId) {
  const client = createClient(supabaseUrl, supabaseKey);
...
const LINEAR_GRAPHQL_URL$1 = "https://api.linear.app/graphql";
...
`;
async function linearGraphQLJson(apiKey, query, variables) {
  const res = await fetch(LINEAR_GRAPHQL_URL$1, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function uploadBufferToLinear(params) {
  ...
  const { ok, json } = await linearGraphQL(apiKey, FILE_UPLOAD_MUTATION, {
    ...
    size: Math.min(Math.floor(size), 2147483647)
  });
  if (!ok || !json) {
...
const _FdR7bb = defineCachedEventHandler(async (event) => {
...
const handlers = [
  ...
  { route: '/**', handler: _lazy_o23dZ7, lazy: true, middleware: false, method: undefined }
];
