import { d as defineEventHandler, r as readBody, t as tryFastPathUnitOnly, g as getNextBookingStep, u as useRuntimeConfig, a as tryShopInfoResponse, b as runWithRetries, p as parseEntityClarifyMessage, h as handleForcedEntityClarify, c as clarifyResponsePayload, s as shopDisambiguationResponsePayload, e as extractReferredEntityPhrase, f as extractBookingTargetFallback, i as resolveBookingTargetFromPhrase, j as getShopById, k as probeReferentPhrase, l as routeReferentFromProbe, m as getDiveSitesForShop, n as getRentalEquipmentForShop, o as getCoursesForShop, q as clampBookingPayloadToNextStep, v as applyInferredCoursesToPayloadIfEligible, w as applyParsedTripDatesToBookingPayload, x as tryParseTripDatesFromMessage, y as inclusiveTripDays, z as tryParseDiverFieldEditIntent, A as snapshotDiverField, B as clearDiverFieldOnCopy, C as buildDiverFieldEditPrompt, D as profileDiverSelectableChipsFromPrefill, E as tryFastPath, F as mergeCollectedIntoBookingPayload, G as buildDiveShopQuery } from '../../nitro/nitro.mjs';
...
function isLongTripConfirmMessage(msg) {
  const t = msg.trim();
  return /^(yes|yeah|yep|correct|right|confirmed|confirm|absolutely|sure)$/i.test(t) || /^that'?s?\s*(right|correct)\b/i.test(t) || /^yes,?\s*(that'?s?\s*)?(is\s*)?correct\b/i.test(t) || /\bthat'?s?\s+correct\s+for\s+my\s+plans\b/i.test(t);
}
function isLongTripRejectMessage(msg) {
  return /^(no|nope)\b/i.test(msg.trim()) || /\b(wrong|not\s+right|different\s+dates)\b/i.test(msg.trim());
}
function formatReplyAfterAppliedTripDates(p, parsedDates, coursesLen, diveSitesLen, coursesDateAckPartsFn, diveSitesLine) {
  var _a, _b;
  ...
  if ((nextAfter == null ? void 0 : nextAfter.step) === "courses" && coursesLen > 0) {
    const parts = coursesDateAckPartsFn(p, parsedDates.startDate, parsedDates.endDate);
    dateStepPreamble = parts.messagePreamble;
    msg = parts.message;
  } else if ((nextAfter == null ? void 0 : nextAfter.step) === "diveSites" && diveSitesLen > 0) {
    dateStepPreamble = `Got it \u2014 ${parsedDates.startDate} to ${parsedDates.endDate}.`;
    msg = ((_a = p.desiredCourses) == null ? void 0 : _a.length) ? `I noted ${p.desiredCourses.join(", ")} from your search. Which dive sites would you like to dive? ${diveSitesLine}` : `Which dive sites would you like to dive? ${diveSitesLine}`;
  } else if ((nextAfter == null ? void 0 : nextAfter.step) === "numberOfDivers") {
    dateStepPreamble = `Got it \u2014 ${parsedDates.startDate} to ${parsedDates.endDate}.`;
    msg = ((_b = p.desiredCourses) == null ? void 0 : _b.length) && coursesLen > 0 ? `I noted ${p.desiredCourses.join(", ")} from your search. How many divers should we book for?` : `How many divers should we book for?`;
  }
...
function orchestratorSplitBookingCopyForStep(next, p, opts) {
  ...
  const di = (_b = next.diverIndex) != null ? _b : 0;
  const diverRow = (_c = p.divers) == null ? void 0 : _c[di];
  const displayName = ((diverRow == null ? void 0 : diverRow.name) || next.diverName || "").trim() || `Diver ${di + 1}`;
...
function isConfirmSendMessage(msg) {
  const t = msg.trim();
  return /^(yes|yeah|yep|ok|okay|sure|send|submit|confirm|go ahead|do it|please send|ready)$/i.test(t) || /^(send|submit)\s+(booking\s+)?(request)?$/i.test(t) || /^(just\s+)?send(?:\s+it)?$/i.test(t);
}
function isSendAnywayMessage(msg) {
  const t = msg.trim();
  return /^(send anyway|still send|send it anyway|yes send anyway|confirm send anyway)$/i.test(t);
}
function isFinishRemainingTasksMessage(msg) {
  const t = msg.trim();
  return /^(finish|finish tasks|finish remaining tasks|complete (the )?(form|tasks)|i('| a)?ll finish|let'?s finish)$/i.test(t);
}
function normalizeBookingPayloadForSendCheck(payload) {
...
const aiSearch_post = defineEventHandler(async (event) => {
  ...
  try {
    ...
    return await runWithRetries(async () => {
      var _a2, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r, _s, _t, _u, _v, _w, _x, _y, _z, _A, _B, _C, _D, _E, _F, _G, _H, _I, _J, _K, _L, _M, _N, _O, _P, _Q, _R, _S, _T, _U, _V, _W, _X, _Y, _Z, __, _$, _aa, _ba, _ca, _da, _ea, _fa;
      ...
      if (clarifyChoice && (pendingEntityClarifyPhrase == null ? void 0 : pendingEntityClarifyPhrase.trim())) {
        ...
        const referredPhrase = (_a2 = extractReferredEntityPhrase(message)) != null ? _a2 : extractBookingTargetFallback(message);
        if (referredPhrase) {
      ...
      if (resolvedShop && (wantsToBook || continuingBooking || resolvedByNamedShop)) {
        let bookingPayload = continuingBooking ? bodyBookingPayload : wantsToBook && bodyPendingPayload ? { ...bodyPendingPayload, shopId: resolvedShop.id } : bodyBookingPayload;
        ...
        if (continuingBooking && bookingPayload) {
          bookingPayload = clampBookingPayloadToNextStep(bookingPayload, {
        ...
        const coursesIntroMessage = (shopName, p) => {
          var _a3;
          if (((_a3 = p.desiredCourses) == null ? void 0 : _a3.length) && p.coursesSelectionComplete === false) {
        ...
        if (startingFreshBooking && noPayloadYet) {
          ...
          if (profilePrefill) {
            fromProfile = {
              name: (_b = profilePrefill.name) != null ? _b : base.name,
              email: (_c = profilePrefill.email) != null ? _c : base.email
            };
          }
          ...
          initialPayload = clampBookingPayloadToNextStep(initialPayload, {
            shopCourseCount: courses.length,
            shopDiveSiteCount: diveSites.length
          });
          ...
          nextHint = getNextBookingStep(initialPayload);
          const firstMessage = (nextHint == null ? void 0 : nextHint.step) === "name" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What's the name for the booking?` : (nextHint == null ? void 0 : nextHint.step) === "email" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What email should we use for the booking?` : (nextHint == null ? void 0 : nextHint.step) === "dates" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What are your trip dates (start and end)?` : (nextHint == null ? void 0 : nextHint.step) === "courses" ? coursesIntroMessage(resolvedShop.business_name, initialPayload) : (nextHint == null ? void 0 : nextHint.step) === "diveSites" ? ((_d = initialPayload.desiredCourses) == null ? void 0 : _d.length) ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. I noted ${initialPayload.desiredCourses.join(", ")} from your search. Which dive sites would you like to dive?` : `Great \u2014 I'll help you book with ${resolvedShop.business_name}. Which dive sites would you like to dive?` : `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What's the name for the booking?`;
          return {
            success: true,
            intent: "booking",
            bookingReady: false,
            ...
            selectableOptions: void 0,
            rentalEquipmentOptions: void 0,
            courseOptions: ((_e = getNextBookingStep(initialPayload)) == null ? void 0 : _e.step) === "courses" && courses.length > 0 ? courses : void 0,
            diveSiteOptions: ((_f = getNextBookingStep(initialPayload)) == null ? void 0 : _f.step) === "diveSites" && diveSites.length > 0 ? diveSites : void 0
          };
        }
        const addGearOptions = (payload) => {
          var _a3;
          return ((_a3 = getNextBookingStep(payload)) == null ? void 0 : _a3.step) === "gear" ? rentalEquipment : void 0;
        };
        const addCourseOptions = (payload) => {
          var _a3;
          return ((_a3 = getNextBookingStep(payload)) == null ? void 0 : _a3.step) === "courses" && courses.length > 0 ? courses : void 0;
        };
        const addDiveSiteOptions = (payload) => {
          var _a3;
          return ((_a3 = getNextBookingStep(payload)) == null ? void 0 : _a3.step) === "diveSites" && diveSites.length > 0 ? diveSites : void 0;
        };
        const hideNoneForGear = (payload) => {
          ...
          const gear = (_b2 = (_a3 = payload.divers) == null ? void 0 : _a3[next.diverIndex]) == null ? void 0 : _b2.gear;
        ...
        if (continuingBooking && !bookingPayload) {
          ...
          if (/continue with this shop|continue booking|proceed with this shop/i.test(msgTrim)) {
            ...
            if (profilePrefill) {
              fromProfile = {
                name: (_g = profilePrefill.name) != null ? _g : base.name,
                email: (_h = profilePrefill.email) != null ? _h : base.email
              };
            }
            let initialPayload = { ...base, ...fromProfile };
            let nextHint = getNextBookingStep(initialPayload);
            if ((nextHint == null ? void 0 : nextHint.step) === "courses" && courses.length === 0) {
              initialPayload = { ...initialPayload, desiredCourses: [] };
              nextHint = getNextBookingStep(initialPayload);
            ...
            nextHint = getNextBookingStep(initialPayload);
            const firstMessage = (nextHint == null ? void 0 : nextHint.step) === "name" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What's the name for the booking?` : (nextHint == null ? void 0 : nextHint.step) === "email" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What email should we use for the booking?` : (nextHint == null ? void 0 : nextHint.step) === "dates" ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What are your trip dates (start and end)?` : (nextHint == null ? void 0 : nextHint.step) === "courses" ? coursesIntroMessage(resolvedShop.business_name, initialPayload) : (nextHint == null ? void 0 : nextHint.step) === "diveSites" ? ((_i = initialPayload.desiredCourses) == null ? void 0 : _i.length) ? `Great \u2014 I'll help you book with ${resolvedShop.business_name}. I noted ${initialPayload.desiredCourses.join(", ")} from your search. Which dive sites would you like to dive?` : `Great \u2014 I'll help you book with ${resolvedShop.business_name}. Which dive sites would you like to dive?` : `Great \u2014 I'll help you book with ${resolvedShop.business_name}. What's the name for the booking?`;
            return {
              success: true,
              intent: "booking",
              bookingReady: false,
              ...
              selectableOptions: void 0,
              rentalEquipmentOptions: void 0,
              courseOptions: ((_j = getNextBookingStep(initialPayload)) == null ? void 0 : _j.step) === "courses" && courses.length > 0 ? courses : void 0,
              diveSiteOptions: ((_k = getNextBookingStep(initialPayload)) == null ? void 0 : _k.step) === "diveSites" && diveSites.length > 0 ? diveSites : void 0
            };
          }
        }
        if (continuingBooking && bookingPayload) {
          ...
          const nextStepBeforeInput = getNextBookingStep(payloadForSendCheck);
          ...
          if (sendIntent || sendAnywayIntent || finishTasksIntent) {
            ...
            if (finishTasksIntent && nextStepBeforeInput) {
              ...
              return {
                ...
                intent: "booking",
                bookingReady: false,
                message: (_l = stepCopy == null ? void 0 : stepCopy.message) != null ? _l : "No problem \u2014 let\u2019s finish the remaining details.",
                ...(stepCopy == null ? void 0 : stepCopy.messagePreamble) ? { messagePreamble: stepCopy.messagePreamble } : {},
                shopId: resolvedShop.id,
                shopName: resolvedShop.business_name,
          ...
          if (((_m = getNextBookingStep(bookingPayload)) == null ? void 0 : _m.step) === "dates") {
            ...
            if (bp.pendingLongTripConfirmation) {
              ...
              if (isLongTripConfirmMessage(msgTrim)) {
                const p = applyParsedTripDatesToBookingPayload(
                  { ...bp, pendingLongTripConfirmation: void 0 },
                  pend,
                  applyTripDatesCtx
                );
              ...
              bp = { ...bp, pendingLongTripConfirmation: void 0 };
            }
            ...
            if (parsedDates) {
              ...
              const p = applyParsedTripDatesToBookingPayload(bp, parsedDates, applyTripDatesCtx);
              const copy = formatReplyAfterAppliedTripDates(
                p,
                parsedDates,
                courses.length,
          ...
          const lastAssistantContent = (_o = (_n = history == null ? void 0 : history.filter((m) => m.role === "assistant").pop()) == null ? void 0 : _n.content) != null ? _o : "";
          const lastWasReadyToSend = /(?:ready to send your booking request|can i send the booking request)/i.test(lastAssistantContent);
          const confirmSend = isConfirmSendMessage(msgTrim) || lastWasReadyToSend && /^(yes|send|submit|confirm|ok)$/i.test(msgTrim);
          ...
          const addGearForName = (_p = addGearForNameMatch == null ? void 0 : addGearForNameMatch[1]) == null ? void 0 : _p.trim();
          if (editEmail || editName || editDates || editGearDiver1 || editGearDiver2 || reviewBooking || addGearForName) {
            ...
            if (addGearForName && ((_q = p.divers) == null ? void 0 : _q.length)) {
            ...
            const numDivers = Math.max(1, (_r = p.numberOfDivers) != null ? _r : 1);
            if (editGearDiver1 && ((_s = p.divers) == null ? void 0 : _s[0])) {
            ...
            if (editGearDiver2 && numDivers >= 2 && ((_t = p.divers) == null ? void 0 : _t[1])) {
            ...
            if (reviewBooking) {
              ...
              const diverLines = (p.divers || []).slice(0, (_u = p.numberOfDivers) != null ? _u : 0).map((d, i) => {
                var _a3;
                const gearList = (((_a3 = d.gear) == null ? void 0 : _a3.length) ? d.gear.map((g) => g.gearType).join(", ") : "none") || "none";
          ...
          const diverFieldEdit = tryParseDiverFieldEditIntent(msgTrim, bookingPayload, {
            currentGearDiverIndex: (nextForDiverEdit == null ? void 0 : nextForDiverEdit.step) === "gear" ? (_v = nextForDiverEdit.diverIndex) != null ? _v : null : null
          });
          if (diverFieldEdit) {
            const numDiversEdit = Math.max(1, (_w = bookingPayload.numberOfDivers) != null ? _w : 1);
          ...
          if (rentalEquipmentNames.length > 0 && msgTrim.length > 0 && (nextStepForGearTap == null ? void 0 : nextStepForGearTap.step) === "gear" && nextStepForGearTap.diverIndex != null) {
            ...
            if (matched) {
              const numDivers = Math.max(1, (_x = bookingPayload.numberOfDivers) != null ? _x : 1);
              ...
              if (targetDiver && !((_y = targetDiver.gear) == null ? void 0 : _y.some((g) => (g.gearType || "").toLowerCase() === msgTrim.toLowerCase()))) {
          ...
          if (((_z = getNextBookingStep(workingPayload)) == null ? void 0 : _z.step) === "courses" && courses.length === 0) {
          ...
          const numDiversForDone = Math.max(1, (_A = bookingPayload.numberOfDivers) != null ? _A : 1);
          const lastDiverForDone = (_B = bookingPayload.divers) == null ? void 0 : _B[numDiversForDone - 1];
          if (((_C = lastDiverForDone == null ? void 0 : lastDiverForDone.gear) == null ? void 0 : _C.length) && (/^(done|that's all|finish|that's it)$/i.test(msgTrim) || msgTrim.toLowerCase() === "none")) {
          ...
          if (lastAssistantContent && /add another diver/i.test(lastAssistantContent) && continuingBooking && bookingPayload) {
            const numDivers = Math.max(1, (_D = bookingPayload.numberOfDivers) != null ? _D : 1);
          ...
          if ((nextStep == null ? void 0 : nextStep.step) === "ready") {
            const confirmSend2 = isConfirmSendMessage(msgTrim);
            if (confirmSend2) {
              const p = { ...bookingPayload, shopId: resolvedShop.id };
              return {
                success: true,
          ...
          if (nextStep) {
            ...
            if (fast) {
              const fp = clampBookingPayloadToNextStep(fast.payload, {
                shopCourseCount: courses.length,
                shopDiveSiteCount: diveSites.length
              });
              const nextAfterFast = (_E = getNextBookingStep(fp)) == null ? void 0 : _E.step;
              ...
              const gearChipsForFast = rentalEquipment.length > 0 ? rentalEquipment : void 0;
              const noRentalGearOptions = ((_F = fast.selectableOptions) == null ? void 0 : _F.length) ? fast.selectableOptions : void 0;
              const showGearChips = noRentalGearOptions ? void 0 : ((_G = getNextBookingStep(fp)) == null ? void 0 : _G.step) === "gear" ? gearChipsForFast : void 0;
              return {
                success: true,
                intent: "booking",
                bookingReady: false,
        ...
        const aiMessage2 = ((_I = (_H = aiData2.choices[0]) == null ? void 0 : _H.message) == null ? void 0 : _I.content) || "";
        ...
        const mergedForGearUi = collectedPayload != null ? collectedPayload : bookingPayload;
        const gearOptionsFromStep = mergedForGearUi ? addGearOptions(mergedForGearUi) : void 0;
        if (gearOptionsFromStep && replyMessage === genericFallback) {
          const numDivers = Math.max(1, (_L = mergedForGearUi == null ? void 0 : mergedForGearUi.numberOfDivers) != null ? _L : 1);
          const divers = (_M = mergedForGearUi == null ? void 0 : mergedForGearUi.divers) != null ? _M : [];
          const idx = (_O = (_N = getNextBookingStep(mergedForGearUi)) == null ? void 0 : _N.diverIndex) != null ? _O : numDivers - 1;
          const nm = ((_Q = (_P = divers[idx]) == null ? void 0 : _P.name) == null ? void 0 : _Q.trim()) || `Diver ${idx + 1}`;
          replyMessage = `Does ${nm} need any rental gear?`;
        }
        const finalGearOptions = gearOptionsFromStep;
        const mergedForDiverChips = (_R = collectedPayload != null ? collectedPayload : bookingPayload) != null ? _R : {};
        const nextHintDiverChips = getNextBookingStep(mergedForDiverChips);
        const profileDiverOptionsFromLlm = (nextHintDiverChips == null ? void 0 : nextHintDiverChips.step) === "diverName" && ((_S = nextHintDiverChips.diverIndex) != null ? _S : 0) >= 1 && profilePrefill ? profileDiverSelectableChipsFromPrefill(profilePrefill, { bookingPayload: mergedForDiverChips }) : void 0;
        ...
        return {
          ...
          rentalEquipmentOptions: (finalGearOptions == null ? void 0 : finalGearOptions.length) ? finalGearOptions : void 0,
      ...
      const isPaginationRequest = paginationPattern.test(message) || next20Pattern.test(message) || listOrShowShopsPattern.test(message);
  ...
  Do not include a MESSAGE. Just return the FILTERS.`;
        try {
          ...
          if (filterResponse.ok) {
            ...
            const filterMessage = ((_U = (_T = filterData.choices[0]) == null ? void 0 : _T.message) == null ? void 0 : _U.content) || "";
              ...
              if (alreadyShown === 0 && (history == null ? void 0 : history.length)) {
                for (let i = 0; i < history.length; i++) {
                  ...
                  if (msg.role === "assistant") {
                    const hasResultsPhrase = ((_V = msg.content) == null ? void 0 : _V.includes("Here are")) || ((_W = msg.content) == null ? void 0 : _W.includes("top results")) || ((_X = msg.content) == null ? void 0 : _X.includes("Here are the"));
                    const isAskingQuestion = ((_Y = msg.content) == null ? void 0 : _Y.includes("What type")) || ((_Z = msg.content) == null ? void 0 : _Z.includes("Would you")) || ((__ = msg.content) == null ? void 0 : __.trim().endsWith("?"));
                    if (hasResultsPhrase && !isAskingQuestion) {
                      const nextN = (_aa = (_$ = msg.content) == null ? void 0 : _$.match(/next (\d+)\s+results?/i)) == null ? void 0 : _aa[1];
      ...
      const aiMessage = ((_ca = (_ba = aiData.choices[0]) == null ? void 0 : _ba.message) == null ? void 0 : _ca.content) || "";
      ...
      if (!((_da = filters.country) == null ? void 0 : _da.trim())) {
  ...
  - One short question only.`;
      const [dbResult, broadeningResult, followUpAiMessage] = await Promise.all([
        ...
        fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          let content = ((_b2 = (_a3 = data.choices[0]) == null ? void 0 : _a3.message) == null ? void 0 : _b2.content) || "";
        ...
        fetch("https://openrouter.ai/api/v1/chat/completions", {
          ...
          if (!res.ok) return "";
          const data = await res.json();
          return ((_c2 = (_b2 = (_a3 = data.choices[0]) == null ? void 0 : _a3.message) == null ? void 0 : _b2.content) == null ? void 0 : _c2.trim()) || "";
        }).catch(() => "")
      ]);
      ...
      if (resultCount <= 2 || wantsMoreOptions) {
        ...
        if ((_ea = broadeningResult.suggestions) == null ? void 0 : _ea.length) {
        ...
        const lastAssistantMessage = ((_fa = history.filter((h) => h.role === "assistant").pop()) == null ? void 0 : _fa.content) || "";
