import { d as defineEventHandler, I as getAuthUser, H as createError, J as getBearerToken, K as createSupabaseClientForUser, u as useRuntimeConfig } from '../../../nitro/nitro.mjs';
...
const index_get = defineEventHandler(async (event) => {
  ...
  const { data: rows, error } = await client.from("booking_submissions").select("id, shop_id, payload, sent_at, created_at").order("sent_at", { ascending: false });
  if (error) {
